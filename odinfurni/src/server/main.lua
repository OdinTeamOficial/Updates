...
--[[vRP._prepare("vRP/furnSqlReady","SELECT * FROM INFORMATION_SCHEMA.TABLES where TABLE_SCHEMA=@dbName AND TABLE_NAME=@tabName")
vRP._prepare("vRP/furnControlAddition","SELECT * FROM "..(Config and Config.SaveToTable or "allhousing"))
vRP._prepare("vRP/furnControlAdditionupdate","UPDATE "..(Config and Config.SaveToTable or "allhousing").." SET furniture=@furniture WHERE id=@id")
vRP._prepare("vRP/SqlCheckfurn","SELECT * FROM information_schema.COLUMNS")
vRP._prepare("vRP/furnset","UPDATE "..(Config and Config.SaveToTable or "allhousing").." SET furniture=@furniture WHERE id=@id")]]
vRP._prepare("vRP/sqlready","SELECT * FROM INFORMATION_SCHEMA.TABLES where TABLE_SCHEMA=@dbName AND TABLE_NAME=@tabName")
vRP._prepare("vRP/controlad","SELECT * FROM "..(Config and Config.SaveToTable or "odinhouse"))
vRP._prepare("vRP/controladupdate","UPDATE "..(Config and Config.SaveToTable or "odinhouse").." SET furniture=@furniture WHERE id=@id")
...
furni.canPlayerAfford = function(source,price)
  if Config.UsingESX then
    ...
    if player.getMoney() >= price then
      if Config["UsingESX_V1.2"] then
        player.removeAccountMoney(Config.CashAccountName,price)
      else
        player.removeMoney(price)
      end
      return true
    elseif player.getAccount(Config.BankAccountName).money >= price then
      player.removeAccountMoney(Config.BankAccountName,price)
      return true
    end
  else
    -- NON ESX USERS FILL IN HERE.
    if vRP.tryFullPayment(vRP.getUserId(source),price) then
      return true
    end
  end
  return false
...
furni.givePlayerMoney = function(source,price)
  if Config.UsingESX then
    ...
    -- NON ESX USERS FILL IN HERE.
    --vRPUtils.giveBank(source,price)
    vRP.giveBankMoney(vRP.getUserId(source),price)
  end
  return false
end

furni.getIdentifier = function(source,ignoreKash)
  if Config.UsingESX then
...
SqlReady = function()
  ...
  if not dbName then
    print("Erro ao conectar á database")
    return
  end

  local dbTable = (Config and Config.SaveToTable or "odinhouse")
  local retData = MySQL.Sync.fetchAll("SELECT * FROM INFORMATION_SCHEMA.TABLES where TABLE_SCHEMA=@dbName AND TABLE_NAME=@tabName",{['@dbName'] = dbName,['@tabName'] = dbTable })
  --SqlFetch("SELECT * FROM INFORMATION_SCHEMA.TABLES where TABLE_SCHEMA=@dbName AND TABLE_NAME=@tabName",{['@dbName'] = dbName,['@tabName'] = dbTable},function(retData)
    if retData and retData[1] then
      ...
      print(GetCurrentResourceName().." Não possui a database necessária.")
    end
  --end)
...
if not Config.Controlesql then
  --[[SqlFetch = function(ssm,spl,callback)
    exports["GHMattiMySQL"]:QueryResultAsync(ssm,spl,callback)
  end

  SqlExecute = function(ssm,spl)
    exports["GHMattiMySQL"]:QueryAsync(ssm,spl)
  end]]

  local dbName = GetDatabaseName()

  if not dbName then
    print("Erro ao conectar a database.")
    return
  end
...
if not Config.ControlAddition then
  furni.placeFurniture = function(source,houseData,itemData,pos,rot,object)
    ...
    if truePrice and furni.canPlayerAfford(source,truePrice) then
      local retData = vRP.query("vRP/controlad", {})
      --SqlFetch("SELECT * FROM "..(Config and Config.SaveToTable or "allhousing"),{},function(retData)
        for _,data in pairs(retData) do
          ...
          if (math.floor(tonumber(houseData.Entry.x)) == math.floor(tonumber(entry.x)) and math.floor(tonumber(houseData.Entry.y)) == math.floor(tonumber(entry.y)) and math.floor(tonumber(houseData.Entry.z)) == math.floor(tonumber(entry.z))) then
            ...
            vRP.execute("vRP/controladupdate", { furniture = json.encode(jTab),id = data.id })
...
if not Config.ControlDeletion then
  furni.deleteFurniture = function(source,houseData,itemData,pos,rot)
    ...
    --SqlFetch("SELECT * FROM "..(Config and Config.SaveToTable or "allhousing"),{},function(retData)
      for _,data in pairs(retData) do
        entry = json.decode(data.entry)
        if (math.floor(tonumber(houseData.Entry.x)) == math.floor(tonumber(entry.x)) and math.floor(tonumber(houseData.Entry.y)) == math.floor(tonumber(entry.y)) and math.floor(tonumber(houseData.Entry.z)) == 
