...
NativeCreateUpgradeMenu = function(d,menu,empty)  
  ...
  for key,price in pairs(sortedTable) do
    ...
    if data.available and d.Shell ~= data.shell then
      ...
      if not empty then 
        _item.Activated = function(...) if CheckLong(d,Labels['LongPlayer']) then UpgradeHouse(d,data); end end
      end
...
NativeCreateSellMenu = function(d,menu)
  ...
  verifyItem.Activated = function() _Pool:CloseAllMenus(); if CheckLong(d,Labels['LongPlayer']) then SellHouse(d); end end
...
CheckLong = function(d,n)
  if Vdist(d.Entry.x, d.Entry.y, d.Entry.z, GetEntityCoords(PlayerPedId())) > 2 then 
    ShowNotification(n)
    return false
  else
    return true
...
NativeEntryOwnerMenu = function(d)
  ...
  if d.Shell then
    ...
    enter.Activated = function(...) if CheckLong(d,Labels['LongPlayer']) then EnterHouse(d) end end
    ...
    if d.Unlocked then
      local lock = NativeUI.CreateItem("Trancar Casa",d.Shell)
      lock.Activated = function(...) if CheckLong(d,Labels['LongPlayer']) then LockHouse(d) end end
      entryMenu:AddItem(lock)
    else
      local unlock = NativeUI.CreateItem("Destrancar Casa",d.Shell)
      unlock.Activated = function(...) if CheckLong(d,Labels['LongPlayer']) then UnlockDoor(d); end end
      entryMenu:AddItem(unlock)
    end
  end

  if d.Garage and Config.MovimentoDaGaragem then
    local move = NativeUI.CreateItem("Mover Garagem",d.Shell)
    move.Activated = function(...) if CheckLong(d,Labels['LongPlayer']) then MoveGarage(d) ; end end
...
NativeEntryOwnedMenu = function(d)
  ...
  if not hasKeys then
    if d.Shell then
      ...
      knock.Activated = function(...) if CheckLong(d,Labels['LongPlayer']) then KnockOnDoor(d); end end
      ...
      if jobName then
        ...
        raid.Activated = function(...) if CheckLong(d,Labels['LongPlayer']) then RaidHouse(d); end end
    ...
    if d.Shell then
      if Config.RouboDeCasas and not isPolice then
        ...
        breakIn.Activated = function(...) if CheckLong(d,Labels['LongPlayer']) then BreakInHouse(d); end end
        entryMenu:AddItem(breakIn)
      end

      if d.Unlocked then
        local enterHouse = NativeUI.CreateItem(Labels['EnterHouse'],d.Shell)
        enterHouse.Activated = function(...) if CheckLong(d,Labels['LongPlayer']) then EnterHouse(d,true); end end
...
NativeEntryEmptyMenu = function(d)
  ...
  buy.Activated = function(...) if CheckLong(d,Labels['LongPlayer']) then BuyHouse(d); end end
  ...
  if d.Shell then
    ...
    visit.Activated = function(...) if CheckLong(d,Labels['LongPlayer']) then ViewHouse(d); end end
