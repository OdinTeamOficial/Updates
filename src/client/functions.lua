...
function GetBlipData(t,zoneCoord)
  if t == "owner" then
    if not Config.EsconderBlipsCasaComprada then
      local color,sprite
      if Config.UsarSpritePorZona then
        local zone = GetZoneAtCoords(zoneCoord.x,zoneCoord.y,zoneCoord.z)
        sprite = (Config.BlipPorZona[zone] and Config.BlipPorZona[zone].BlipDonoZona or Config.BlipDonoCasa)
      else
        sprite = Config.BlipDonoCasa
      end
      if Config.UsarCorPorZona then
        local zone = GetZoneAtCoords(zoneCoord.x,zoneCoord.y,zoneCoord.z)
        color = (Config.CorBlipZona[zone] and Config.CorBlipZona[zone].CorDonoZoa or Config.CorBlipDonoCasa)
      else
        color = Config.CorBlipDonoCasa
      end
      return color,sprite
    else
      return false
  ...
  elseif t == "owned" then
    if not Config.EsconderBlipsCasaOutros then
      local color,sprite
      if Config.UsarSpritePorZona then
        local zone = GetZoneAtCoords(zoneCoord.x,zoneCoord.y,zoneCoord.z)
        sprite = (Config.BlipPorZona[zone] and Config.BlipPorZona[zone].BlipCasaCompradaZona or Config.BlipCasaComprada)
      else
        sprite = Config.BlipCasaComprada
      end
      if Config.UsarCorPorZona then
        local zone = GetZoneAtCoords(zoneCoord.x,zoneCoord.y,zoneCoord.z)
        color = (Config.CorBlipZona[zone] and Config.CorBlipZona[zone].CorCasaCompradaZona or Config.CorBlipCasaComprada)
      else
        color = Config.CorBlipCasaComprada
      end
      return color,sprite
    else
      return false
  ...
  elseif t == "empty" then
    if not Config.EsconderBlipsCasaVazia then
      local color,sprite
      if Config.UsarSpritePorZona then
        local zone = GetZoneAtCoords(zoneCoord.x,zoneCoord.y,zoneCoord.z)
        sprite = (Config.BlipPorZona[zone] and Config.BlipPorZona[zone].BlipCasaVaziaZona or Config.BlipCasaVazia)
      else
        sprite = Config.BlipCasaVazia
      end
      if Config.UsarCorPorZona then
        local zone = GetZoneAtCoords(zoneCoord.x,zoneCoord.y,zoneCoord.z)
        color = (Config.CorBlipZona[zone] and Config.CorBlipZona[zone].CorCasaVaziaZona or Config.CorBlipCasaVazia)
      else
        color = Config.CorBlipCasaVazia
      end
      return color,sprite
    else
      return false
...
function RefreshBlips()
  local identifier = Callback("Allhousing:GetPlId")
  if Config.UsarBlips then
    for _,house in pairs(Houses) do
      ...
      if house.Owned and (tonumber(house.Owner) == identifier) then
        text = Labels['MyHouse']
...
function Callback(event,...)
  ...
  Citizen.Wait(math.random(1,10)*100)
