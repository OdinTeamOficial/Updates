...
ShellModels = {  
  ...
  ApartmentV11  = "playerhouse_tier3",
...
ShellOffsets = {  
  ...
  ApartmentV11   = {exit = vector4(-2.6,7.4,15.0,0.0)},
  ...
  Citizen.CreateThread(function()
    ...
    for _,house in ipairs(Houses) do
      if check_coords[house.Entry] then
        print("Dupla Entrada na localização","Entrada: "..tostring(house.Entry))
        return
      else
    ...
    if not error_out then
      print("A tabela de Casas foi carregada com sucesso.")
    end
  end)
end
