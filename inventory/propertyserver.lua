...
function src.openChest(homeName)
	...
	if user_id then
		...
		return hsinventory,myinventory,vRP.getInventoryWeight(user_id),vRP.getInventoryMaxWeight(user_id),vRP.computeItemsWeight(result),parseInt(Config.espacobau)
	end
	return false
end
		...
		if user_id and actived[parseInt(user_id)] == 0 or not actived[parseInt(user_id)] then
			...
			if items then
				if parseInt(amount) > 0 then
					...
					if new_weight <= parseInt(Config.espacobau) then
