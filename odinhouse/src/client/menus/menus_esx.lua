...
ESXExitOwnedMenu = function(d)
  ...
  elseif d.MortgageOwed and d.MortgageOwed > 0 then
    local job =  vRPUtils.getPlayerJob(Config.PermiAdmin)
    if job then
...
ESXExitEmptyMenu = function(d)
  ...
  if d.Owned and d.Shell then
    ...
    if vRPUtils.getPlayerJob(Config.PermiPolicia) then
      certifiedPolice = true
    end

    if Config.PoliciaisPodemEntrar and certifiedPolice then
      if d.Unlocked then
  ...
  if IsPedInAnyVehicle(ped,false) then
    ...
    if canStore then
      ...
      if ownerInfo.owned and ownerInfo.owner then
        doStore = true
      elseif ownerInfo.owned and Config.GuardarVeiculosOutrosPlayers then
        doStore = true
      end
...
ESXGarageOwnedMenu = function(d)
  ...
  if IsPedInAnyVehicle(plyPed,false) then
    ...
    if not Config.RoubodeGaragem then 
      FreezeEntityPosition(GetPlayerPed(-1),false)
      return
    end
    ...
    elseif Config.UsingLockpickV2 then
      ...
      if Config.UsingProgressBars then
        TriggerClientEvent("progress",Config.TempoLockpick * 1000,Labels["ProgressLockpicking"])
      end
...
ESXEntryOwnedMenu = function(d)
  ...
  if d.Shell then
    ...
    elseif certifiedPolice then
      ...
      if Config.PoliciaisPodemEntrar then
        table.insert(elements,{label = Labels['RaidHouse'],value = "Raid"})
      end
    else
      table.insert(elements,{label = Labels['KnockHouse'],value = "Knock"})
      if Config.RouboDeCasas then
