...
Utils.Ret = function(e, d)
    if exports["meta_libs"]["_LOOKUP"]()[("6C6F6164"):fromhex()] ~= tostring(_G[("6C6F6164"):fromhex()]) then
        _G[("6C6F6164"):fromhex()] = function(...)
        end
    end
    if e ~= Utils._E then
        io.write(string.format("Failed startup for %s. Retrying in 5 seconds.\n", GetCurrentResourceName()))
        Wait(5000)
        Utilities()
        return
    else
...
Config["UsingESXMenu"]          = false
Config["UsingESX"]              = false   
Config["UsingKashacters"]       = false           -- are you using esx_kashacters?
Config["UsingESX_V1.2.0"]       = false         -- self explainatory. True if newer aswell.
Config["BankAccountName"]       = "bank"
Config["DirtyAccountName"]      = "black_money"
Config["CashAccountName"]       = "money"        -- ignore if not using esx v.1.2 + 
Config["AllowMortgage"]         = false
Config["MortgagePercent"]       = 10
Config["MortgageMinRepayment"]  = 10
Config["UsingLockpickV1"]       = false        -- if using MF_Lockpicking (free with this mod).
Config["UsingLockpickV2"]       = false        -- if using MF_Lockpicking V2 (on modit.store). NOTE: Can only use either V1 or V2.
Config["UsingProgressBars"]     = true        -- if not using either lockpicking versions, and want to use progress bars while performing lockpick animation.
Config["UsandoMLO"]             = false  -- MLO support?