...
vRP._prepare("odinhouse/createtable",[[
  CREATE TABLE IF NOT EXISTS `odinhouse` (
    `id` int(11) NOT NULL,
...
Citizen.CreateThread(function()
  vRP.execute("odinhouse/createtable")
...
PurchaseHouse = function(house)
  ...
  if purchase then
    ...
    if #rows <= Config.Max_Homes - 1 then
      ...
      v.Owned = false
      TriggerClientEvent("Allhousing:SyncHouse",-1,v)
      TriggerClientEvent("Allhousing:NotifyPlayer", _source, string.format(Labels['HouseLimit'],Config.Max_Homes))
    end
    SetTimeout(3*1000,function()
      purchase = true
    end)
  else
    TriggerClientEvent("Allhousing:NotifyPlayer", _source, Labels['AnyTry'])
  end
end
  ...
  if Houses and type(Houses) == "table" then
    for k1,v1 in pairs(Houses) do
      ...
      if not foundMatch then
        ...
        newHouses[curInDb[foundMatch].id].Bau = (curInDb[foundMatch].Bau or 100)
      end
    end
  end
  if curInDb and type(curInDb) == "table" then
...
CreateHouse = function(house)
  ...
  local bau_length = vRP.prompt(source, Labels['SetStoragequantity'], '0')
  
  bau_length = math.max(1,(tonumber(bau_length) and tonumber(bau_length) > 0 and tonumber(bau_length) or 0))
  house.Bau = bau_length
  if jobName then 
...
SetInventory = function(house,inventoryLocation)
  ...
  if v.Owned == true and (parseInt(v.Owner) == parseInt(vRP.getUserId(_source))) then
...
RegisterCallback("Allhousing:GetPlId", GetCreatePerm)
