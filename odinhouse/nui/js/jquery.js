	...
	document.onkeyup = function(data){
		if (data.which == 27){
			...
			$.post("http://odinhouse/chestClose");
		}
	};
});
		...
		drop: function(event,ui){
			...
			if ( tInv === "invleft" ) {
				if ( origin === "invleft" ) {
					...
					$.post("http://odinhouse/populateSlot",JSON.stringify({
						item: itemData.key,
						slot: itemData.slot,
						target: target,
					...
					$.post("http://odinhouse/takeItem",JSON.stringify({
						item: itemData.key,
						slot: target,
						amount: parseInt($(".amount").val())
					}))
				...
				if ( origin === "invleft" ) {
					...
					$.post("http://odinhouse/storeItem",JSON.stringify({
						item: itemData.key,
						slot: itemData.slot,
						amount: parseInt($(".amount").val())
					}))
		...
		drop: function(event,ui){
			...
			if ( tInv === "invleft" ) {
				if ( origin === "invleft" ) {
					...
					$.post("http://odinhouse/updateSlot",JSON.stringify({
						item: itemData.key,
						slot: itemData.slot,
						target: target,
						amount: parseInt($(".amount").val())
					...
					$.post("http://odinhouse/sumSlot",JSON.stringify({
						item: itemData.key,
						slot: target,
						amount: parseInt($(".amount").val())
					}))
...
const updateVault = () => {
	...
	$.post("http://odinhouse/requestVault",JSON.stringify({}),(data) => {
